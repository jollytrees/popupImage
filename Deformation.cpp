...
void computeDeformationCostsApproximately(const vector<int> &patch_index_mask, const int IMAGE_WIDTH, const int IMAGE_HEIGHT, const int FOLD_LINE_WINDOW_WIDTH, const int FOLD_LINE_WINDOW_HEIGHT, map<int, map<int, map<int, int> > > &patch_pair_fold_line_positions, map<int, map<int, double> > &fold_line_x_score_map)
{
  int num_patches = 0;
  ...
  for (int patch_index_1 = 0; patch_index_1 < num_patches; patch_index_1++) {
	...
	continue;
      patch_pair_fold_line_positions[patch_pair_index / num_patches][patch_pair_index % num_patches][fold_line_index] = fold_line_position;
      fold_line_index++;
  ...
  for (int patch_index = 0; patch_index < num_patches; patch_index++) {
    if (patch_min_xs[patch_index] > IMAGE_WIDTH / 2 || patch_max_xs[patch_index] < IMAGE_WIDTH / 2)
      continue;
    for (set<int>::const_iterator background_patch_it = background_patches.begin(); background_patch_it != background_patches.end(); background_patch_it++) {
      if (patch_pair_fold_line_positions[*background_patch_it].count(patch_index) == 0) {
	int patch_pair_index = *background_patch_it * num_patches + patch_index;
	if (fold_line_score_pairs[patch_pair_index].first != -1) {
	  int fold_line_position = fold_line_score_pairs[patch_pair_index].first;
	  patch_pair_fold_line_positions[patch_pair_index / num_patches][patch_pair_index % num_patches][fold_line_index] = fold_line_position;
	  fold_line_index++;
	  patch_right_fold_line_xs[patch_pair_index / num_patches].push_back(fold_line_position % IMAGE_WIDTH);
      ...
      if (patch_pair_fold_line_positions[patch_index].count(*background_patch_it) == 0) {
        int patch_pair_index = patch_index * num_patches + *background_patch_it;
        if (fold_line_score_pairs[patch_pair_index].first != -1) {
          int fold_line_position = fold_line_score_pairs[patch_pair_index].first;
          patch_pair_fold_line_positions[patch_pair_index / num_patches][patch_pair_index % num_patches][fold_line_index] = fold_line_position;
          fold_line_index++;
  ...
  for (int patch_index = 0; patch_index < num_patches; patch_index++) {
	...
	patch_pair_fold_line_positions[neighbor_patch_index][patch_index][fold_line_index] = get<1>(highest_score_fold_line_tuple);
	fold_line_index++;
    ...
    if (has_right_fold_line == false) {
      ...
      if (get<0>(highest_score_fold_line_tuple) != -1) {
        int neighbor_patch_index = get<0>(highest_score_fold_line_tuple);
        patch_pair_fold_line_positions[patch_index][neighbor_patch_index][fold_line_index] = get<1>(highest_score_fold_line_tuple);
	fold_line_index++;
  ...
  fold_line_x_score_map.clear();
  for (map<int, map<int, map<int, int> > >::const_iterator patch_it_1 = patch_pair_fold_line_positions.begin(); patch_it_1 != patch_pair_fold_line_positions.end(); patch_it_1++) {
    for (map<int, map<int, int> >::const_iterator patch_it_2 = patch_it_1->second.begin(); patch_it_2 != patch_it_1->second.end(); patch_it_2++) {
      int patch_pair_index = patch_it_1->first * num_patches + patch_it_2->first;
      for (map<int, vector<double> >::const_iterator x_it = fold_line_x_scores[patch_pair_index].begin(); x_it != fold_line_x_scores[patch_pair_index].end(); x_it++) {
        ...
        for (vector<double>::const_iterator score_it = x_it->second.begin(); score_it != x_it->second.end(); score_it++)
          if (*score_it > max_score)
            max_score = *score_it;
	for (map<int, int>::const_iterator fold_line_it = patch_it_2->second.begin(); fold_line_it != patch_it_2->second.end(); fold_line_it++)
	  fold_line_x_score_map[fold_line_it->first][x_it->first] = max_score;
      }
    }
  }
}

void findAllFoldLines(const vector<int> &patch_index_mask, const int IMAGE_WIDTH, const int IMAGE_HEIGHT, const int MIDDLE_FOLD_LINE_X, const int FOLD_LINE_WINDOW_WIDTH, const int FOLD_LINE_WINDOW_HEIGHT, const map<int, map<int, map<int, int> > > &patch_pair_fold_line_positions, PopupGraph &popup_graph)
{
  popup_graph.background_patches.clear();
  popup_graph.background_patches.insert(patch_index_mask[0]);
  
  popup_graph.num_original_patches = 0;
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++)
    popup_graph.num_original_patches = max(patch_index_mask[pixel] + 1, popup_graph.num_original_patches);
  
  popup_graph.pixel_line_segment_indices.assign(IMAGE_WIDTH * IMAGE_HEIGHT, -1);
  ...
  for (int x = 0; x < IMAGE_WIDTH; x++) {
	...
	line_segment_index++;
      popup_graph.pixel_line_segment_indices[y * IMAGE_WIDTH + x] = line_segment_index;
      previous_patch_index = patch_index_mask[y * IMAGE_WIDTH + x];
    }
  }
  
  int num_line_segments = line_segment_index + 1;
  vector<int> original_fold_line_patch_pairs;
  vector<vector<int> > patch_fold_line_segment_indices(popup_graph.num_original_patches);
  vector<vector<int> > patch_fold_line_index_map(popup_graph.num_original_patches);
  
  popup_graph.line_segment_fold_line_indices.assign(num_line_segments, -1);
  vector<int> line_segment_left_fold_lines(num_line_segments, 0);
  vector<int> line_segment_right_fold_lines(num_line_segments, 0);
  map<int, vector<int> > original_patch_fold_lines;
  
  popup_graph.num_original_fold_lines = 0;
  set<int> background_patch_left_fold_lines;
  set<int> background_patch_right_fold_lines;
  for (map<int, map<int, map<int, int> > >::const_iterator patch_it_1 = patch_pair_fold_line_positions.begin(); patch_it_1 != patch_pair_fold_line_positions.end(); patch_it_1++) {
    for (map<int, map<int, int> >::const_iterator patch_it_2 = patch_it_1->second.begin(); patch_it_2 != patch_it_1->second.end(); patch_it_2++) {
      for (map<int, int>::const_iterator fold_line_it = patch_it_2->second.begin(); fold_line_it != patch_it_2->second.end(); fold_line_it++) {
	int patch_index_1 = patch_it_1->first;
	int patch_index_2 = patch_it_2->first;
	int original_fold_line_index = fold_line_it->first;
	popup_graph.num_original_fold_lines = max(original_fold_line_index + 1, popup_graph.num_original_fold_lines);
	int fold_line_x = fold_line_it->second % IMAGE_WIDTH;
	int intersection_y = fold_line_it->second / IMAGE_WIDTH;
	set<int> line_segment_indices_1;
	set<int> line_segment_indices_2;
	for (int delta_x = 0; delta_x >= -FOLD_LINE_WINDOW_WIDTH / 2; delta_x--) {
	  ...
	  for (int delta_y = -FOLD_LINE_WINDOW_HEIGHT / 2; delta_y <= FOLD_LINE_WINDOW_HEIGHT / 2; delta_y++) {
		...
		line_segments.insert(popup_graph.pixel_line_segment_indices[pixel]);
	    }
	  }
	  if (line_segments.size() >= 1) {
	    line_segment_indices_1 = line_segments;
	    break;
	  }
	}
	for (int delta_x = 0; delta_x <= FOLD_LINE_WINDOW_WIDTH / 2; delta_x++) {
          ...
          if (line_segments.size() >= 1) {
            line_segment_indices_2 = line_segments;
	    break;
	  }
        }
	if (line_segment_indices_1.size() == 0 || line_segment_indices_2.size() == 0)
	  continue;

	if (popup_graph.background_patches.count(patch_index_1) == 0) {
	  for (set<int>::const_iterator line_segment_it = line_segment_indices_1.begin(); line_segment_it != line_segment_indices_1.end(); line_segment_it++) {
	    line_segment_left_fold_lines[*line_segment_it] = line_segment_right_fold_lines[*line_segment_it] = pow(2, original_patch_fold_lines[patch_index_1].size());
	    popup_graph.line_segment_fold_line_indices[*line_segment_it] = original_fold_line_index;
	  }
	} else {
	  background_patch_right_fold_lines.insert(original_fold_line_index);
          // if (fold_line_x < MIDDLE_FOLD_LINE_X)
	  //   background_patch_right_fold_lines.insert(original_fold_line_index * 2);
          // else
	  //   background_patch_left_fold_lines.insert(original_fold_line_index * 2 * 1);
        }
	if (popup_graph.background_patches.count(patch_index_2) == 0) {
	  for (set<int>::const_iterator line_segment_it = line_segment_indices_2.begin(); line_segment_it != line_segment_indices_2.end(); line_segment_it++) {
	    line_segment_left_fold_lines[*line_segment_it] = line_segment_right_fold_lines[*line_segment_it] = pow(2, original_patch_fold_lines[patch_index_2].size());
	    popup_graph.line_segment_fold_line_indices[*line_segment_it] = original_fold_line_index;
	  }
	} else {
	  background_patch_left_fold_lines.insert(original_fold_line_index);
	  // if (fold_line_x < MIDDLE_FOLD_LINE_X)
          //   background_patch_right_fold_lines.insert(original_fold_line_index * 2);
          // else
	  //background_patch_left_fold_lines.insert(original_fold_line_index * 2 + 1);
	}
        original_patch_fold_lines[patch_index_1].push_back(original_fold_line_index);
        original_patch_fold_lines[patch_index_2].push_back(original_fold_line_index);
	...
	popup_graph.original_patch_fold_lines[patch_index_1][patch_index_2].insert(original_fold_line_index);
      }
    }
  }

  // cout << num_original_fold_lines << endl;
  // for (int patch_index = 0; patch_index < num_original_patches; patch_index++) {
  //   cout << patch_index << '\t' << original_patch_fold_lines[patch_index].size() << endl;
  //   //for (vector<int>::const_iterator fold_line_it = original_patch_fold_lines[patch_index].begin(); fold_line_it != original_patch_fold_lines[patch_index].end(); fold_line_it++)
  //   //cout << *fold_line_it << '\t';
  //   //cout << endl;
  // }
  ...
  while (true) {
	...
	continue;
      int line_segment_index = popup_graph.pixel_line_segment_indices[pixel];
      int neighbor_line_segment_index = popup_graph.pixel_line_segment_indices[neighbor_pixel];
  ...
  vector<int> line_segment_original_patch_indices(num_line_segments);
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
    line_segment_original_patch_indices[popup_graph.pixel_line_segment_indices[pixel]] = patch_index_mask[pixel];
  }
  ...
  int valid_patch_index = popup_graph.num_original_fold_lines;
  int invalid_patch_index = -1;
  int background_patch_index = -1;
  for (int line_segment_index = 0; line_segment_index < num_line_segments; line_segment_index++) {
    if (popup_graph.line_segment_fold_line_indices[line_segment_index] != -1)        
      continue;
    ...
    if (popup_graph.background_patches.count(line_segment_original_patch_indices[line_segment_index]) > 0) {
      if (new_patch_index_map.count(index) == 0) {
	new_patch_index_map[index] = valid_patch_index;
	background_patch_index = valid_patch_index;
	valid_patch_index++;
      }
      continue;
    }
  ...
  //cout << valid_patch_index << '\t' << invalid_patch_index << endl;
  //exit(1);
  
  vector<int> line_segment_new_patch_indices(num_line_segments, -1);
  map<int, int> patch_original_patch_index_map;
  for (int line_segment_index = 0; line_segment_index < num_line_segments; line_segment_index++) {
    if (popup_graph.line_segment_fold_line_indices[line_segment_index] != -1)
      continue;
    ...
    if (popup_graph.background_patches.count(line_segment_original_patch_indices[line_segment_index]) > 0) {
      line_segment_new_patch_indices[line_segment_index] = new_patch_index_map[index];
      patch_original_patch_index_map[new_patch_index_map[index]] = line_segment_original_patch_indices[line_segment_index];
      continue;
    }
  ...
  if (true) {
    Mat original_graph_image = Mat::zeros(IMAGE_HEIGHT, IMAGE_WIDTH, CV_8UC3);
    ...
    for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
      int patch_index = line_segment_original_patch_indices[popup_graph.pixel_line_segment_indices[pixel]];
      // if (patch_index < 0) {
      //   // if (line_segment_fold_line_indices[line_segment_indices[pixel]] == -1) {
    ...
    for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
      if (popup_graph.line_segment_fold_line_indices[popup_graph.pixel_line_segment_indices[pixel]] == -1) {
        continue;
      }
      int fold_line_index = popup_graph.line_segment_fold_line_indices[popup_graph.pixel_line_segment_indices[pixel]];
      //if (fold_line_index == 2) {
      //cout << pixel % IMAGE_WIDTH << '\t' << pixel / IMAGE_WIDTH << endl;
      //exit(1);
      //}
      if (fold_line_index < popup_graph.num_original_fold_lines)
  ...
  if (true) {
    Mat new_graph_image = Mat::zeros(IMAGE_HEIGHT, IMAGE_WIDTH, CV_8UC3);
    //Mat original_fold_line_image(IMAGE_HEIGHT, IMAGE_WIDTH, CV_8UC3);
    map<int, Vec3b> patch_color_table;
    map<int, vector<int> > patch_pixels;
    for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
      int patch_index = line_segment_new_patch_indices[popup_graph.pixel_line_segment_indices[pixel]];
      // if (patch_index < 0) {
      //   // if (line_segment_fold_line_indices[line_segment_indices[pixel]] == -1) {
      //   //   cout << "invalid patch mask" << endl;
      //   //   exit(1);
  ...
  map<int, set<int> > patch_left_patches;
  map<int, set<int> > patch_right_patches;
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
    if (pixel % IMAGE_WIDTH == IMAGE_WIDTH - 1)
      continue;
    int neighbor_pixel = pixel + 1;
    if (patch_index_mask[pixel] != patch_index_mask[neighbor_pixel])
      continue;
    int left_line_segment_index = popup_graph.pixel_line_segment_indices[pixel];
    int right_line_segment_index = popup_graph.pixel_line_segment_indices[neighbor_pixel];
    int left_new_patch_index = line_segment_new_patch_indices[left_line_segment_index];
    int right_new_patch_index = line_segment_new_patch_indices[right_line_segment_index];
    int left_fold_line_index = popup_graph.line_segment_fold_line_indices[left_line_segment_index];
    int right_fold_line_index = popup_graph.line_segment_fold_line_indices[right_line_segment_index];
    ...
    if (left_new_patch_index >= 0 && right_fold_line_index >= 0) {
      patch_right_fold_lines[left_new_patch_index].insert(right_fold_line_index);
      fold_line_left_patches[right_fold_line_index].insert(left_new_patch_index);
    }
    if (right_new_patch_index >= 0 && left_fold_line_index >= 0) {
      patch_left_fold_lines[right_new_patch_index].insert(left_fold_line_index);
      fold_line_right_patches[left_fold_line_index].insert(right_new_patch_index);
    }
    if (left_new_patch_index >= 0 && right_new_patch_index >= 0 && left_new_patch_index != right_new_patch_index) {
      patch_right_patches[left_new_patch_index].insert(right_new_patch_index);
      patch_left_patches[right_new_patch_index].insert(left_new_patch_index);
    }
  }
  ...
  for (map<int, set<int> >::const_iterator patch_it = patch_left_fold_lines.begin(); patch_it != patch_left_fold_lines.end(); patch_it++)
    for (set<int>::const_iterator fold_line_it = patch_it->second.begin(); fold_line_it != patch_it->second.end(); fold_line_it++)
      popup_graph.fold_line_pairs.push_back(make_pair(*fold_line_it, patch_it->first));
  for (map<int, set<int> >::const_iterator patch_it = patch_right_fold_lines.begin(); patch_it != patch_right_fold_lines.end(); patch_it++)
    for (set<int>::const_iterator fold_line_it = patch_it->second.begin(); fold_line_it != patch_it->second.end(); fold_line_it++)
      popup_graph.fold_line_pairs.push_back(make_pair(patch_it->first, *fold_line_it));
  for (map<int, set<int> >::const_iterator patch_it = patch_left_patches.begin(); patch_it != patch_left_patches.end(); patch_it++)
    for (set<int>::const_iterator neighbor_patch_it = patch_it->second.begin(); neighbor_patch_it != patch_it->second.end(); neighbor_patch_it++)
      popup_graph.fold_line_pairs.push_back(make_pair(*neighbor_patch_it, patch_it->first));
  for (map<int, set<int> >::const_iterator patch_it = patch_right_fold_lines.begin(); patch_it != patch_right_fold_lines.end(); patch_it++)
    for (set<int>::const_iterator neighbor_patch_it = patch_it->second.begin(); neighbor_patch_it != patch_it->second.end(); neighbor_patch_it++)
      popup_graph.fold_line_pairs.push_back(make_pair(patch_it->first, *neighbor_patch_it));

  {
    int left_border_fold_line_index = valid_patch_index;
    for (set<int>::const_iterator fold_line_it = background_patch_right_fold_lines.begin(); fold_line_it != background_patch_right_fold_lines.end(); fold_line_it++)
      popup_graph.fold_line_pairs.push_back(make_pair(left_border_fold_line_index, *fold_line_it));
    popup_graph.fold_line_pairs.push_back(make_pair(left_border_fold_line_index, background_patch_index));
  }
  {
    int right_border_fold_line_index = valid_patch_index + 1;
    for (set<int>::const_iterator fold_line_it = background_patch_left_fold_lines.begin(); fold_line_it != background_patch_left_fold_lines.end(); fold_line_it++)
      popup_graph.fold_line_pairs.push_back(make_pair(*fold_line_it, right_border_fold_line_index));
    popup_graph.fold_line_pairs.push_back(make_pair(background_patch_index, right_border_fold_line_index));
  }
  
  popup_graph.num_fold_lines = valid_patch_index + 2;

  for (int line_segment_index = 0; line_segment_index < num_line_segments; line_segment_index++)
    if (popup_graph.line_segment_fold_line_indices[line_segment_index] == -1)
      popup_graph.line_segment_fold_line_indices[line_segment_index] = line_segment_new_patch_indices[line_segment_index];


  map<int, map<int, double> > original_x_score_map = popup_graph.fold_line_x_score_map;
  popup_graph.fold_line_x_score_map.clear();
  for (int fold_line_index = 0; fold_line_index < popup_graph.num_original_fold_lines; fold_line_index++)
    popup_graph.fold_line_x_score_map[fold_line_index] = original_x_score_map[fold_line_index];
  for (int pixel = 0; pixel < IMAGE_WIDTH * IMAGE_HEIGHT; pixel++) {
    int line_segment_index = popup_graph.pixel_line_segment_indices[pixel];
    int fold_line_index = popup_graph.line_segment_fold_line_indices[line_segment_index];
    if (fold_line_index >= popup_graph.num_original_fold_lines)
      popup_graph.fold_line_x_score_map[fold_line_index][pixel % IMAGE_WIDTH] = 1;
  }
  popup_graph.fold_line_x_score_map[valid_patch_index][0] = 1;
  popup_graph.fold_line_x_score_map[valid_patch_index + 1][IMAGE_WIDTH - 1] = 1;


  {
    map<int, vector<pair<int, int> > > original_patch_fold_line_pairs;
    {
      map<int, set<int> > fold_line_original_patches_map;
      for (int line_segment_index = 0; line_segment_index < num_line_segments; line_segment_index++)
        if (popup_graph.line_segment_fold_line_indices[line_segment_index] >= 0)
	  fold_line_original_patches_map[popup_graph.line_segment_fold_line_indices[line_segment_index]].insert(line_segment_original_patch_indices[line_segment_index]);
	...
	for (set<int>::const_iterator left_patch_it = fold_line_original_patches_map[left_fold_line_index].begin(); left_patch_it != fold_line_original_patches_map[left_fold_line_index].end(); left_patch_it++)
	  for (set<int>::const_iterator right_patch_it = fold_line_original_patches_map[right_fold_line_index].begin(); right_patch_it != fold_line_original_patches_map[right_fold_line_index].end(); right_patch_it++)
	    if (*left_patch_it == *right_patch_it)
	      original_patch_fold_line_pairs[*left_patch_it].push_back(*fold_line_pair_it);
      }
    }

    for (map<int, vector<pair<int, int> > >::const_iterator patch_it = original_patch_fold_line_pairs.begin(); patch_it != original_patch_fold_line_pairs.end(); patch_it++) {
      map<int, map<int, vector<int> > > patch_fold_line_left_paths;
      map<int, map<int, vector<int> > > patch_fold_line_right_paths;
      vector<pair<int, int> > same_patch_fold_line_pairs = patch_it->second;
	...
	for (vector<pair<int, int> >::const_iterator fold_line_pair_it = same_patch_fold_line_pairs.begin(); fold_line_pair_it != same_patch_fold_line_pairs.end(); fold_line_pair_it++) {
	  ...
	  if ((left_fold_line_index < popup_graph.num_original_fold_lines && left_fold_line_index % 2 == 0) || (right_fold_line_index < popup_graph.num_original_fold_lines && right_fold_line_index % 2 == 1))
	  ...
	  // for (vector<int>::const_iterator fold_line_it = fold_lines.begin(); fold_line_it != fold_lines.end(); fold_line_it++) {
	  //   if (patch_left_fold_line_paths[right_patch_index].count(*fold_line_it) > 0)
	  //     continue;
	  //   if (patch_left_fold_line_paths[left_patch_index].count(*fold_line_it) > 0) {
	  //     vector<int> path = patch_left_fold_line_paths[left_patch_index][*fold_line_it];
	  //     path.push_back(left_patch_index);
	  //     patch_left_fold_line_paths[right_patch_index][*fold_line_it] = path;
	  //     has_change = true;
	  //   } else if (patch_right_fold_line_paths[left_patch_index].count(*fold_line_it) > 0) {
	  //     patch_left_fold_line_paths[right_patch_index][*fold_line_it] = patch_right_fold_line_paths[left_patch_index][*fold_line_it];
	  //     has_change = true;
	  //   }
	  // }
	}
	...
	for (map<int, vector<int> >::const_iterator fold_line_it_2 = fold_line_it_1->second.begin(); fold_line_it_2 != fold_line_it_1->second.end(); fold_line_it_2++)
	  popup_graph.fold_line_left_paths[fold_line_it_1->first][fold_line_it_2->first] = fold_line_it_2->second;
      for (map<int, map<int, vector<int> > >::const_iterator fold_line_it_1 = patch_fold_line_right_paths.begin(); fold_line_it_1 != patch_fold_line_right_paths.end(); fold_line_it_1++)
        for (map<int, vector<int> >::const_iterator fold_line_it_2 = fold_line_it_1->second.begin(); fold_line_it_2 != fold_line_it_1->second.end(); fold_line_it_2++)
          popup_graph.fold_line_right_paths[fold_line_it_1->first][fold_line_it_2->first] = fold_line_it_2->second;
    }
    
    {
      int left_border_fold_line_index = valid_patch_index;
      for (set<int>::const_iterator fold_line_it = background_patch_right_fold_lines.begin(); fold_line_it != background_patch_right_fold_lines.end(); fold_line_it++) {
	popup_graph.fold_line_right_paths[left_border_fold_line_index][*fold_line_it] = vector<int>();
	popup_graph.fold_line_left_paths[*fold_line_it][left_border_fold_line_index] = vector<int>();
      }
      popup_graph.fold_line_right_paths[left_border_fold_line_index][background_patch_index] = vector<int>();
      popup_graph.fold_line_left_paths[background_patch_index][left_border_fold_line_index] = vector<int>();
    }
    {
      int right_border_fold_line_index = valid_patch_index + 1;
      for (set<int>::const_iterator fold_line_it = background_patch_left_fold_lines.begin(); fold_line_it != background_patch_left_fold_lines.end(); fold_line_it++) {
	popup_graph.fold_line_left_paths[right_border_fold_line_index][*fold_line_it] = vector<int>();
	popup_graph.fold_line_right_paths[*fold_line_it][right_border_fold_line_index] = vector<int>();
      }
      popup_graph.fold_line_left_paths[right_border_fold_line_index][background_patch_index] = vector<int>();
      popup_graph.fold_line_right_paths[background_patch_index][right_border_fold_line_index] = vector<int>();
    }
  }
}

  // {
//   int new_fold_line_index = num_original_fold_lines;
...
//     // 	has_change = true;
//     // 	for (map<int, set<int> >::iterator fold_line_it = fold_line_left_patches.begin(); fold_line_it != fold_line_left_patches.end(); fold_line_it++)
//     // 	  fold_line_it->second.erose(new_patch_index);
//     //   }
//     // }
...
int main(int argc, char *argv[])
{
  ...
  map<int, map<int, map<int, int> > > patch_pair_fold_line_positions;
  map<int, map<int, double> > fold_line_x_score_map;
  ...
  patch_index_mask = zoomMask(patch_index_mask, IMAGE_WIDTH, IMAGE_HEIGHT, NEW_IMAGE_WIDTH, NEW_IMAGE_HEIGHT);
  ...
  //void findAllFoldLines(const vector<int> &patch_index_mask, const int IMAGE_WIDTH, const int IMAGE_HEIGHT, const int FOLD_LINE_WINDOW_WIDTH, const int FOLD_LINE_WINDOW_HEIGHT, const vector<vector<int> > &patch_pair_fold_line_positions, vector<map<int, double> > &fold_line_x_score_map, vector<int> &fold_line_pairs)
  PopupGraph popup_graph;
  popup_graph.fold_line_x_score_map = fold_line_x_score_map;
  findAllFoldLines(patch_index_mask, NEW_IMAGE_WIDTH, NEW_IMAGE_HEIGHT, NEW_IMAGE_WIDTH / 2, FOLD_LINE_WINDOW_WIDTH, FOLD_LINE_WINDOW_HEIGHT, patch_pair_fold_line_positions, popup_graph);
  ...
  sort(popup_graph.fold_line_pairs.begin(), popup_graph.fold_line_pairs.end());
  popup_graph.fold_line_pairs.erase(unique(popup_graph.fold_line_pairs.begin(), popup_graph.fold_line_pairs.end()), popup_graph.fold_line_pairs.end());
  if (false) {
    //    cout << popup_graph.num_original_fold_lines << '\t' << popup_graph.num_fold_lines << endl;
    for (vector<pair<int, int> >::const_iterator fold_line_pair_it = popup_graph.fold_line_pairs.begin(); fold_line_pair_it != popup_graph.fold_line_pairs.end(); fold_line_pair_it++) {
      int left_fold_line_index = fold_line_pair_it->first;
      int right_fold_line_index = fold_line_pair_it->second;
      cout << left_fold_line_index << '\t' << right_fold_line_index << endl;
  ...
  for (int fold_line_index = 0; fold_line_index < popup_graph.num_fold_lines; fold_line_index++)
    for (int other_fold_line_index = 0; other_fold_line_index < popup_graph.num_fold_lines; other_fold_line_index++)
      if ((popup_graph.fold_line_left_paths.count(fold_line_index) > 0 && popup_graph.fold_line_left_paths.at(fold_line_index).count(other_fold_line_index) > 0) && (popup_graph.fold_line_right_paths.count(fold_line_index) > 0 && popup_graph.fold_line_right_paths.at(fold_line_index).count(other_fold_line_index) > 0))
	invalid_fold_line_pairs[fold_line_index].insert(other_fold_line_index);

  map<int, map<int, vector<int> > > new_fold_line_left_paths;
  for (int fold_line_index = 0; fold_line_index < popup_graph.num_fold_lines; fold_line_index++)
    //if (fold_line_index >= popup_graph.num_original_fold_lines || fold_line_index % 2 == 0)
      for (map<int, vector<int> >::const_iterator fold_line_it = popup_graph.fold_line_left_paths[fold_line_index].begin(); fold_line_it != popup_graph.fold_line_left_paths[fold_line_index].end(); fold_line_it++)
	//	if ((fold_line_it->first >= popup_graph.num_original_fold_lines || fold_line_it->first % 2 == 1) && invalid_fold_line_pairs[fold_line_index].count(fold_line_it->first) == 0)
  ...
  popup_graph.fold_line_left_paths = new_fold_line_left_paths;
  
  map<int, map<int, vector<int> > > new_fold_line_right_paths;
  for (int fold_line_index = 0; fold_line_index < popup_graph.num_fold_lines; fold_line_index++)
    //if (fold_line_index >= popup_graph.num_original_fold_lines || fold_line_index % 2 == 1)
      for (map<int, vector<int> >::const_iterator fold_line_it = popup_graph.fold_line_right_paths[fold_line_index].begin(); fold_line_it != popup_graph.fold_line_right_paths[fold_line_index].end(); fold_line_it++)
        //if ((fold_line_it->first >= popup_graph.num_original_fold_lines || fold_line_it->first % 2 == 0) && invalid_fold_line_pairs[fold_line_index].count(fold_line_it->first) == 0)
	if (invalid_fold_line_pairs[fold_line_index].count(fold_line_it->first) == 0)
          new_fold_line_right_paths[fold_line_index][fold_line_it->first] = fold_line_it->second;
  popup_graph.fold_line_right_paths = new_fold_line_right_paths;
  
  if (false) {
    for (int fold_line_index = 0; fold_line_index < popup_graph.num_fold_lines; fold_line_index++) {
      cout << "fold line index: " << fold_line_index << endl;
      cout << popup_graph.fold_line_left_paths.count(fold_line_index) << '\t' << popup_graph.fold_line_right_paths.count(fold_line_index) << endl;
      for (map<int, vector<int> >::const_iterator fold_line_it = popup_graph.fold_line_left_paths[fold_line_index].begin(); fold_line_it != popup_graph.fold_line_left_paths[fold_line_index].end(); fold_line_it++) {
  ...
  optimizeFoldLines(NEW_IMAGE_WIDTH, NEW_IMAGE_HEIGHT, popup_graph);
