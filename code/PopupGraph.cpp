...
namespace Popup
{
  ...
  void PopupGraph::findOriginalFoldLines()
  {
	...
	//suppress other fold lines 1) between same pair of patches, 2) has opposite direction, 3) in the same valley, and 4) has smaller score
	  ...
	  if (other_fold_line_it->score > fold_line_it->score)
	    continue;
	  if (left_patch_fold_line_counter[other_fold_line_it->original_patch_pair.first] <= 1 || right_patch_fold_line_counter[fold_line_it->original_patch_pair.second] <= 1)
	    continue;
  ...
  void PopupGraph::addOptimizedInfo(const PopupGraph &optimized_popup_graph)
  {
	...
	fold_lines_[fold_line_it->first].optimized_flag = true;      
    ...
    // fold_lines_[getBorderFoldLineIndices().first].optimized_flag = true;
    ...
    // fold_lines_[getBorderFoldLineIndices().second].optimized_flag = true;    
    ...
    // 	cout << "island fold line: " << fold_line_it - fold_lines_.begin() << '\t' << fold_line_it->original_patch_pair.first << '\t' << fold_line_it->original_patch_pair.second << endl;
